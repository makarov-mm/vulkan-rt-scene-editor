...
// GDI+ (ships with Windows) for loading the PNG toolbar icons. Its headers
// expect the min/max macros that NOMINMAX removed, so alias the std ones.
#include <algorithm>
...
static const int ADDABLE_MESHES = 8;
...
private:
    ...
    void onMouseMove(int x, int y) {
        ...
        if (mode == MODE_SELECT) {
            if (dragMoved) {
                marqueeOn = true;
                mq1.x = x; mq1.y = y;   // picked up by updateUniforms -> drawn by the shader
            }
    ...
    void createWindow(HINSTANCE hInst) {
        ...
        RECT rect = { 0, 0, (LONG)WIDTH, (LONG)(TB_H + HEIGHT) };
        AdjustWindowRect(&rect, style, FALSE);

        hwnd = CreateWindowEx(0, wc.lpszClassName,
            "Vulkan Ray Tracing - Scene Editor", style,
    ...
    void createDescriptors() {
        ...
        writes[7].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER; writes[7].pBufferInfo = &oInfo;
